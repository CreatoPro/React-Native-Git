...
const Chat = () => {
  ...
  const initConnection = async (token) => {
    console.log("shreesat", config.CHAT_WEBSOCKET, token);
  ...
  const requestToken = (selectedUsername, isModerator, selectedAvatar) => {
    ...
    axios
      ...
      .then((response) => {
        console.log("shree", response);
        setChatToken(response.data.chatToken);
        initConnection(response.data.chatToken);
      })
      .catch((error) => {
        setChatToken(null);
  ...
  const renderErrorMessage = (errorMessage) => {
    return (
      <View>
        <Text key={errorMessage.timestamp}>error</Text>
      </View>
    );
  };
  ...
  const renderSuccessMessage = (successMessage) => {
    return (
      <View>
        <Text key={successMessage.timestamp}>Success</Text>
      </View>
    );
  };
  ...
  const renderStickerMessage = (message) => (
    <View key={message.timestamp}>
      <View key={message.timestamp}>
        <Image source={message.avatar} />
        <Text>Username</Text>
        <Image source={message.sticker} />
      </View>
  ...
  return (
    <View style={{ height: "100%", width: "auto", paddingTop: 100 }}>
      <View>
        <Text>Amazon IVS Chat Web Demo</Text>
      </View>
      <View>
        <View>
          ...
          <View>
            <View>
              ...
              <View style={{ height: "100%" }}>
                ...
                {!username && (
                  <FieldSet>
                    <Button title="wuadnjaw" onPress={handleOnClick}></Button>
                  </FieldSet>
                )}
              </View>
            </View>
